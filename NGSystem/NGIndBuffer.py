...
class NGIndBuffer(ElectricNode):
	"""
	This class represents NG system independent buffers.
	Attributes:
		bufNum:			buffer number (positive integer)
		bbufType: 		buffer type
	...
	def __init__(self):
		"""
		Construct a new buffer with all parameters set to none type
		"""
	...
	def add_xml_child_hfgt(self, parent):
		...
		for k1 in self.attrib_ref:
			method_port = ET.SubElement(indBuffer, 'MethodxPort', OrderedDict([('name', 'store'), ('operand', k1), ('output', k1), ('origin', self.nodeName), ('dest', self.nodeName), ('ref', k1),('status','true'),('controller',', '.join(self.controller))]))
